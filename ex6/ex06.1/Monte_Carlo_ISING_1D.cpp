...
void Measure(int itemp)
{
	...
	//cycle over spins
  	for (int i=0; i<nspin; ++i)
  	{
     	        u += -J * s[itemp][i] * s[itemp][Pbc(i+1)] - 0.5 * h * (s[itemp][i] + s[itemp][Pbc(i+1)]); //energia interna
		sm += s[itemp][i];
		
  	}
  	walker[iu] = u; // metto l'energia tot nella componenti iu di walker
	walker[ic] = u*u;
	walker[im] = sm;
	walker[ix] = sm*sm;
...
void Reset(int iblk) //Reset block averages
{
   ...
   if(iblk == 1)
   {
       ...
       {
           glob_av[i] = 0; //accumulatore di blocco delle medie
           glob_av2[i] = 0; // accumulatore di blocco dei termini quadratici
       }
   }

   for(int i=0; i<n_props; ++i)
   {
     blk_av[i] = 0; //accumulatore delle mosse accettate e rifiutate
   }
   blk_norm = 0;
   attempted = 0;
   accepted = 0;
