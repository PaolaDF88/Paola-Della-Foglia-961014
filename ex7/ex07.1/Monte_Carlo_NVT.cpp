...
{ 
	...
	system("rm -rf *gofr* *gave* *epot* *pres* Pression.dat PotentialEnergy.dat");
	
	ofstream WriteEnergy, WritePression;

	WriteEnergy.open("Potentialenergy.dat", ofstream::app);
	WritePression.open("Pression.dat", ofstream::app);
	
	Input(WriteEnergy, WritePression); //Inizialization	
  	...
  	for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  	{
		...
		{
		  	Move();
		  	Measure(WriteEnergy, WritePression);
...
void Input(ofstream& WriteEnergy, ofstream& WritePression)
{
...
void Move(void)
{
  ...
  //Metropolis test
    p = exp(beta*(energy_old-energy_new)); //uguale a scrivere p=exp(-beta(energy_new-energy_old))
    if(p >= rnd.Rannyu())  
    {
    //Update
       x[o] = xnew;
...
void Measure(ofstream& WriteEnergy, ofstream& WritePression)
{
	...
	WriteEnergy << walker[iv]/(double)npart + vtail << endl;
	WritePression << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
...
//g(r)
	...
	for (int k=igofr; k<igofr+nbins; ++k)
	{
		...
		if(iblk == nblk)
		{
			double err_g=Error(glob_av[k],glob_av2[k],iblk);
			Gave << r+0.5*bin_size <<" "<< glob_av[k]/(double)nblk <<" "<< err_g << endl;
		}
	}
