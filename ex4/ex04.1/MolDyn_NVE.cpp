...
void Input(Random rnd){ //Prepare all stuff for the simulation
	...
	vec_gofr.assign(igofr+nbins, 0.);
	vec_gofr_2.assign(igofr+nbins, 0.);
	
	double sumv2 = 0.0, fs;
	
	if(cont && rescale) { //continue=1 (true) restart=0 (true) ----> restart ultima volta
	...
	} else if (cont && !rescale) { //continue=1 (true) restart=1 (false) ---> continua 2volta
