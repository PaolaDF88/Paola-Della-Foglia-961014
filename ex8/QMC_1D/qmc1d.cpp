...
#include <fstream>
#include <cmath>
#include <TRandom3.h> 	//TRandom3 funzione generatore di numeri casuali
#include "constants.h"
#include "functions.h"
...
{
        // equilibrazione
        readInput();  	//funzione che legge l'input
	initialize();   // inizializzazione dell'input
...
opportunely initialized by the initialize() function. */
	for(int i=0;i<equilibration;i++)  //ciclo di equilibrazione
	{
		if(PIGS)   // only a PIGS polymer has a start and an end. PIGS=1 se T=0 altrimenti PIGS=0 per T qualsiasi
		{
			brownianMotion(LEFT); // prendo il polimero e gli faccio fare moto browniano a sx e a dx
...
void initialize()
{
	...
	if(PIGS)
		...
		dtau = hbar/(boltzmann*temperature*timeslices); //dtau=beta/n
	
	acceptedTranslations=0; //vettori accumulatori
...
double variationalWaveFunction_second(double v)
{
	...
	double Derprimoexp = (pow(v-mu, 2.) - sigma*sigma ) * exp1 / pow(sigma, 4.);
	double exp2 = exp(-pow(v+mu, 2.)/(2.*sigma*sigma));
	double Dersecondoexp = (pow(v+mu, 2.) - sigma*sigma ) * exp2 / pow(sigma, 4.);
	double Derivata_seconda = Derprimoexp+Dersecondoexp;
	return Derivata_seconda;
}
